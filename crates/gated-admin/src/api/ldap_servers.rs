...
use gated_common::{GatedError, Secret};
use gated_db_entities::LdapServer;
use gated_ldap::LdapUsernameAttribute;
...
impl ImportApi {
    ...
    ) -> Result<ImportLdapUsersResponse, GatedError> {
        let db = db.read().await;
        let Some(server) = LdapServer::Entity::find_by_id(id.0).one(&*db).await? else {
            return Ok(ImportLdapUsersResponse::NotFound);
        };
        let ldap_config = gated_ldap::LdapConfig::try_from(&server)?;
        let all_users = gated_ldap::list_users(&ldap_config).await?;
        let mut imported = Vec::new();
        for dn in &body.dns {
            if let Some(user) = all_users.iter().find(|u| &u.dn == dn) {
                let existing = gated_db_entities::User::Entity::find()
                    .filter(gated_db_entities::User::Column::Username.eq(&user.username))
                    .one(&*db)
                    .await?;
                if existing.is_none() {
                    let values = gated_db_entities::User::ActiveModel {
                        id: Set(Uuid::new_v4()),
                        username: Set(user.username.clone()),
                        credential_policy: Set(serde_json::to_value(
                            gated_common::UserRequireCredentialsPolicy::default(),
                        )?),
...
impl From<gated_ldap::LdapUser> for LdapUserResponse {
    fn from(user: gated_ldap::LdapUser) -> Self {
        Self {
...
impl ListApi {
    ...
    async fn api_get_all_ldap_servers(
        ...
        _sec_scheme: AnySecurityScheme,
    ) -> Result<GetLdapServersResponse, GatedError> {
        let db = db.read().await;
        ...
        let servers = query.all(&*db).await.map_err(GatedError::from)?;
    ...
    async fn api_create_ldap_server(
        ...
        _sec_scheme: AnySecurityScheme,
    ) -> Result<CreateLdapServerResponse, GatedError> {
        ...
        let ldap_config = gated_ldap::LdapConfig {
        ...
        // Discover base DNs
        let base_dns = gated_ldap::discover_base_dns(&ldap_config).await?;
        ...
        let server = values.insert(&*db).await.map_err(GatedError::from)?;
    ...
    async fn api_test_ldap_server(
        ...
        _sec_scheme: AnySecurityScheme,
    ) -> Result<TestLdapServerConnectionResponse, GatedError> {
        let ldap_config = gated_ldap::LdapConfig {
            host: body.host.clone(),
            port: body.port as u16,
            bind_dn: body.bind_dn.clone(),
        ...
        match gated_ldap::test_connection(&ldap_config).await {
            Ok(_) => {
                // Try to discover base DNs
                let base_dns = gated_ldap::discover_base_dns(&ldap_config).await.ok();
...
#[allow(
    ...
    reason = "ApiResponse is constructed at most once per request; 224-byte variant is immaterial"
)]
...
impl DetailApi {
    ...
    async fn api_get_ldap_server(
        ...
        id: Path<Uuid>,
        _sec_scheme: AnySecurityScheme,
    ) -> Result<GetLdapServerResponse, GatedError> {
        let db = db.read().await;
    ...
    async fn api_update_ldap_server(
        ...
        _sec_scheme: AnySecurityScheme,
    ) -> Result<UpdateLdapServerResponse, GatedError> {
        let db = db.read().await;
        ...
        let ldap_config = gated_ldap::LdapConfig {
            ...
            bind_password: body
                ...
                .unwrap_or_else(|| {
                    // `model` was produced by `server.into()` a few lines above
                    // and is either `Unchanged(..)` or `Set(..)` for this
                    // field — never `NotSet`, so `ActiveValue::unwrap()` won't
                    // panic.
                    #[allow(
                        clippy::unwrap_used,
                        reason = "ActiveValue is Set/Unchanged here, never NotSet"
                    )]
        ...
        if let Ok(base_dns) = gated_ldap::discover_base_dns(&ldap_config).await {
    ...
    async fn api_delete_ldap_server(
        ...
        id: Path<Uuid>,
        _sec_scheme: AnySecurityScheme,
    ) -> Result<DeleteLdapServerResponse, GatedError> {
        let db = db.read().await;
        ...
        server.delete(&*db).await.map_err(GatedError::from)?;
...
impl QueryApi {
    ...
    async fn api_get_ldap_users(
        ...
        id: Path<Uuid>,
        _sec_scheme: AnySecurityScheme,
    ) -> Result<GetLdapUsersResponse, GatedError> {
        let db = db.read().await;
        ...
        let ldap_config = gated_ldap::LdapConfig::try_from(&server)?;
        let users = match gated_ldap::list_users(&ldap_config).await {
            Ok(users) => users,
            Err(e) => {
