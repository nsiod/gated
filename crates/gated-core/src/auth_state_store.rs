...
impl AuthStateStore {
    ...
    pub async fn create(
    ...
    ) -> Result<(Uuid, Arc<Mutex<AuthState>>), GatedError> {
        let id = Uuid::new_v4();
        ...
        let Some(user) = self
            ...
            return Err(GatedError::UserNotFound(username.into()));
        };
        ...
        #[allow(clippy::unwrap_used, reason = "inserted on the previous line")]
