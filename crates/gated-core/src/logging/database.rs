...
pub fn install_database_logger(database: Arc<RwLock<DatabaseConnection>>) {
    tokio::spawn(async move {
        #[allow(
            clippy::expect_used,
            reason = "LOG_SENDER is set once during install_logging; install_database_logger is only called after that"
        )]
