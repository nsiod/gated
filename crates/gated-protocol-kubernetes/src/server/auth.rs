...
use gated_common::{Target, TargetKubernetesOptions, TargetOptions, User};
use gated_core::{ConfigProvider, Services, State};
use gated_db_entities::{CertificateCredential, CertificateRevocation};
use poem::Request;
...
pub fn create_authenticated_client(
    ...
    if let Some(ca_pem) = &k8s_options.ca_certificate {
        let ca_pem = ca_pem.expose_secret();
        if !ca_pem.trim().is_empty() {
            let mut cert_count = 0usize;
            for pem in Pem::iter_from_buffer(ca_pem.as_bytes()) {
                let pem = pem.context("parsing Kubernetes upstream CA PEM bundle")?;
                if pem.label != "CERTIFICATE" {
                    continue;
                }
                let certificate = reqwest::Certificate::from_der(&pem.contents)
                    .context("parsing Kubernetes upstream CA certificate")?;
                client_builder = client_builder.add_root_certificate(certificate);
                cert_count += 1;
            }

            if cert_count == 0 {
                anyhow::bail!("Kubernetes upstream CA bundle contains no certificates");
            }
        }
    }

    match &k8s_options.auth {
        gated_common::KubernetesTargetAuth::Token(auth) => {
        ...
        gated_common::KubernetesTargetAuth::Certificate(auth) => {
...
pub async fn validate_client_certificate(
    ...
    let cert_credentials = CertificateCredential::Entity::find()
        .find_with_related(gated_db_entities::User::Entity)
        .all(&*db)
        .await?;
