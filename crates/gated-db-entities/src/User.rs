use gated_common::{GatedError, User, UserDetails};
...
impl TryFrom<Model> for User {
    ...
    fn try_from(model: Model) -> Result<Self, GatedError> {
        Ok(User {
            id: model.id,
...
impl Model {
    pub async fn load_details(self, db: &DatabaseConnection) -> Result<UserDetails, GatedError> {
        let roles: Vec<String> = self
            .find_related(Role::Entity)
            .all(db)
            .await?
            .into_iter()
            .map(Into::<gated_common::Role>::into)
            .map(|x| x.name)
            .collect();
