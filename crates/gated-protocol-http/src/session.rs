...
use crate::session_handle::{
    GatedServerHandleFromRequest, HttpSessionHandle, SessionHandleCommand,
};
...
pub struct SessionStore {
    session_handles: HashMap<SessionId, Arc<Mutex<GatedServerHandle>>>,
...
impl SessionStore {
    ...
    pub async fn create_handle_for(
        &mut self,
        req: &Request,
    ) -> poem::Result<GatedServerHandleFromRequest> {
        let session = <&Session>::from_request_without_body(req).await?;
    ...
    pub fn handle_for(&self, session: &Session) -> Option<Arc<Mutex<GatedServerHandle>>> {
        session
