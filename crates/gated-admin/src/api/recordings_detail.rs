...
use gated_core::recordings::{AsciiCast, SessionRecordings, TerminalRecordingItem};
use gated_db_entities::Recording::{self, RecordingKind};
use gated_protocol_kubernetes::recording::{
...
impl Api {
    ...
    #[oai(
        path = "/recordings/:id/api",
        method = "get",
        operation_id = "get_api_recording"
    )]
    async fn api_get_recording_api(
        &self,
        db: Data<&Arc<RwLock<DatabaseConnection>>>,
        recordings: Data<&Arc<Mutex<SessionRecordings>>>,
        id: Path<Uuid>,
    ) -> poem::Result<GetApiRecordingResponse> {
        let db = db.read().await;

        let recording = Recording::Entity::find_by_id(id.0)
            .filter(Recording::Column::Kind.eq(RecordingKind::Api))
            .one(&*db)
            .await
            .map_err(InternalServerError)?;
        ...
        Ok(GetApiRecordingResponse::Ok(Json(content)))
    }
}
