...
use gated_common::helpers::websocket::pump_websocket;
use gated_common::http_headers::DONT_FORWARD_HEADERS;
use gated_common::{GatedError, SessionId, TargetKubernetesOptions, TargetOptions};
use gated_core::logging::http::{
    get_client_ip, log_request_error, log_request_result, span_for_request,
};
use gated_core::recordings::{SessionRecordings, TerminalRecorder, TerminalRecordingStreamId};
use gated_core::{Services, State};
...
pub async fn handle_api_request(
    ...
    let (session_id, log_span) = {
        let handle: tokio::sync::MutexGuard<'_, gated_core::GatedServerHandle> =
            handle.lock().await;
...
async fn _handle_normal_request_inner(
    ...
    debug!(
        ...
        match &k8s_options.auth {
            gated_common::KubernetesTargetAuth::Token(_) => "Token",
            gated_common::KubernetesTargetAuth::Certificate(_) => "Certificate",
        }
    );
...
async fn run_websocket_recording(mut recorder: TerminalRecorder, mut rx: mpsc::Receiver<Vec<u8>>) {
    while let Some(data) = rx.recv().await {
        let Some((&msg_type, rest)) = data.split_first() else {
            continue;
        };
