...
use futures::pin_mut;
use gated_common::{SSHTargetAuth, SessionId, TargetSSHOptions};
            ...
            tokio::select! {
                ...
                session = &mut fut_connect => {
                        ...
                        SSHTargetAuth::PublicKey(_) => {
                            ...
                            for key in keys.into_iter() {
                                ...
                                if auth_result {
                                    ...
                                    auth_error_msg = Some("Public key authentication was rejected by the SSH target".to_string());
                                }
                            }
                        }
                    }
                    ...
                    if !auth_result {
                        ...
                        error!(%reason, "Gated could not authenticate with SSH target");
                        let _ = session
